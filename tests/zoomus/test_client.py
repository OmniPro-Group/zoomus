...
class ZoomClientTestCase(unittest.TestCase):
    def test_init_sets_config(self):
        ...
        self.assertEqual(
            client.config,
            {
                ...
                "account_id": "ACCOUNT",
                "data_type": "json",
                "token": util.generate_jwt("KEY", "SECRET", "ACCOUNT"),
    ...
    def test_can_set_api_version_to_1(self):
        client = ZoomClient("KEY", "SECRET", "ACCOUNT", version=util.API_VERSION_1)
    ...
    def test_can_set_base_uri_to_gdpr(self):
        client = ZoomClient("KEY", "SECRET", "ACCOUNT", base_uri=API_BASE_URIS[util.API_GDPR])
        self.assertEqual(client.config["version"], util.API_VERSION_2)
        for key in client.components.keys():
            self.assertEqual(
    ...
    def test_can_set_custom_base_uri(self):
        client = ZoomClient(
            "KEY", "SECRET", "ACCOUNT", version=util.API_VERSION_1, base_uri="https://www.test.com"
        )
        self.assertEqual(client.config["version"], util.API_VERSION_1)
        for key in client.components.keys():
    ...
    def test_can_get_client_id(self):
        client = ZoomClient("KEY", "SECRET", "ACCOUNT")
        self.assertEqual(client.client_id, "KEY")

    def test_can_set_client_id(self):
        client = ZoomClient("KEY", "SECRET", "ACCOUNT")
        client.client_id = "NEW-KEY"
        self.assertEqual(client.client_id, "NEW-KEY")

    def test_can_get_client_secret(self):
        client = ZoomClient("KEY", "SECRET", "ACCOUNT")
        self.assertEqual(client.client_secret, "SECRET")

    def test_can_set_client_secret(self):
        client = ZoomClient("KEY", "SECRET", "ACCOUNT")
    ...
    def test_can_use_client_with_context(self):
        with ZoomClient("KEY", "SECRET", "ACCOUNT") as client:
    ...
    def test_refresh_token_replaces_config_token_with_new_jwt(self, mock_jwt):
        ...
        mock_jwt.assert_called_with("KEY", "SECRET", "ACCOUNT")
